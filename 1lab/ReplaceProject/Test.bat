...
REM Work with empty file
%program% test_input_output\input.txt "%TEMP%\output.txt" g u
if ERRORLEVEL 1 GOTO error
FC.exe test_input_output\output.txt "%TEMP%\output.txt" > nul 
if ERRORLEVEL 1 GOTO error
ECHO Test 3 successful

REM Work with empty file
%program% test_input_output\input.txt "%TEMP%\empty_output.txt" a b
if ERRORLEVEL 1 GOTO error
FC.exe test_input_output\input.txt "%TEMP%\empty_output.txt" > nul 
if ERRORLEVEL 1 GOTO error
